...
def train_net(net, trainset, valset_list, testset_list, args, ema_net=None):
    ...
    criterion_ce = BinaryCrossEntropyLoss(class_num=args.tn).to(args.gpu)
    criterion_dl = BinaryDiceLoss(class_num=args.tn).to(args.gpu)
    criterion_mod = nn.CrossEntropyLoss(ignore_index=-1).to(args.gpu)

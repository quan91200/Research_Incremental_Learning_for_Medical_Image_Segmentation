...
def get_cuda_bare_metal_version(cuda_dir):
    ...
    if not os.path.exists(nvcc_path):
        print(f"[Warning] nvcc not found at {nvcc_path}. Skipping CUDA version detection.")
        return "nvcc not found", None
...
def check_cuda_torch_binary_vs_bare_metal(cuda_dir):
    if "--disable_cuda_ext" in sys.argv:
        print(">> Skipping CUDA compatibility check (--disable_cuda_ext active)")
        return
...
if not torch.cuda.is_available():
    ...
    if os.environ.get("TORCH_CUDA_ARCH_LIST", None) is None and CUDA_HOME is not None:
        ...
        if bare_metal_version is None:
            bare_metal_version = Version("0.0")
